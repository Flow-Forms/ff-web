...
- #### Send Back -
	...
	- **Last step** - sends the submission back to the user/s from the previous flow step. 
		If the previous step's type was: 
	...
	Next, the **Message** text box will appear for you to explain why it is being sent back. You must enter a message in order to select the final **Send Back** button. You can also click **Cancel** to return to the **Available Actions** menu without proceeding. 
	
	When **Send Back** is selected, the form's submitter will receive a notification that their form's status is now **Sent Back.** The user that was selected for the submission to be sent back to will also receive a notification that includes your message and a link to the submission.
...
- #### + Add Note
	...
	*See the main **+ Add Note** section below, under its own heading, for more information on adding notes.*
...
- **Deleting Your Notes** -  When viewing all notes from the slide-out column, or the {{icon:arrow-top-right-on-square size-4 inline-block align-text-center}} (Pop-out) view, any note you contributed will appear with a trash can ({{icon:trash size-4 inline-block align-text-center}}) icon beside it that allows you to delete any note you contributed. Admin can also delete notes added by any user. 
