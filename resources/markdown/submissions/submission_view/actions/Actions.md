...
- ### Send Back -
	With this option, you can choose to return the submission to a user, or users, from a previous step in the flow or to the initial submitter. The submission will then be pending on that user's action for it to progress again in its flow toward finalization. **Note** - You cannot send back a form to an *anonymous user*, which is anyone that does not have a user account in Flow Forms.
