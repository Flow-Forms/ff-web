...
By default, your search results are displayed with the submission that has had the most recent action taken (or "status update") at the top, and on, in descending order to the oldest submissions. *This sort order can be altered in "[Sort Submissions](Submissions/Display_Options#Sort%20Submissions) {{icon:bars-arrow-down size-5 inline-block align-text-center}}" within the control panel.*
...
- What you searched for isn't highlighted anywhere on a presented submission result.  This could be because your [display](Submissions/Display_Options) option is set to "Collapse All {{icon:arrows-pointing-in size-5 inline-block align-text-center}}" and the form element that matches your search is only visible in expanded view. Select the "Expand All {{icon:arrows-pointing-out size-5 inline-block align-text-center}}" display option from the submissions control panel, or click "{{icon:arrows-pointing-out size-5 inline-block align-text-center}} show more" in the individual submission's header. Another reason could be that the search result was not an exact match to what you typed in the search bar (the previously mentioned punctuation could be to blame).
